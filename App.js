...
export default function App() {
  ...
  addHeart = (value, color) => {
    setHearts([...hearts, { id: heartCount, right: -50, value:value, color:color }]);
    heartCount++;
  }
  ...
  fetchData = async () => {
    ...
    if(users.length > 0){
      if (users.length < 100) {
        ...
        for(let i = 0; i < 100; i++){
          let updateData = { name:users[i].name, score:users[i].score, id:i };
  ...
  onUserInputChange = inputText => {
    ...
    if(v === typable){
      ...
      setTimeout(() => { addHeart('+'+bonus, 'purple') }, 1);
      setTimeout(() => { addHeart('+1秒', 'orange') }, 200);
  ...
  return (
    ...
    <Start startScreen={startScreen} toFalseStartScreen={toFalseStartScreen}/>
      <View style={styles.barContainer}><ProgressBar myHealth={myHealth} enemyHealth={enemyHealth} /></View>
      ...
      <Preview text={text} typable={typable} userInput={userInput}
        finished={finished}
        />
        {/* <Text>{capital}</Text> */}
        <TypeText
        ...
        onPress={onRestart}
        ></Button>
        <Speed symbols={symbols} completed={completed} miss={miss} seconds={sec} symbols={symbols} score={score} finished={finished} scoreData={scoreData} ranked={ranked} toFalseRanked={toFalseRanked} toTrueRanked={toTrueRanked} toTrueName={toTrueName} toFalseName={toFalseName} name={name} toTrueList={toTrueList} toFalseList={toFalseList} list={list} top={top} setTop={setTop} leastScore={leastScore} fetchData={fetchData}
        />
        <Ranking scoreData={scoreData} score={score} fetchData={fetchData} dataLength={dataLength} toFalseRanked={toFalseRanked} toTrueRanked={toTrueRanked} ranked={ranked} toTrueName={toTrueName} toFalseName={toFalseName} toTrueName={toTrueName} toFalseName={toFalseName} toTrueList={toTrueList} toFalseList={toFalseList} list={list} leastScore={leastScore} />
      </View>
